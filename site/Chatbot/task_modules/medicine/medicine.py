...
class MedicalListener(Task):
    ...
    def debug(self,log=None):
        ...
        for k,v in self.symptom_dic.items():
            if v:
                if log is None:
                    print("[DEBUG]: %s" % k.encode('utf-8').strip())
                else:
                    log.write("[Get-Attr]: %s" % k.encode('utf-8').strip())
    ...
    def hard_extract(self, keywords):
        ...
        while True:
            ...
            if domain is not None:
                print("[DEBUG]: hard_extract %s" % domain.encode('utf-8').strip())
    ...
    def soft_extract(self, keywords):
        ...
        while True:
            ...
            print("[DEBUG]: DEC %s" % description_domain.encode('utf-8').strip())
            print("[DEBUG]: SUB %s" % subject_domain.encode('utf-8').strip())
        return sd_list
